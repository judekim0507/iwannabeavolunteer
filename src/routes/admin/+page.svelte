<script lang="ts">
  ...
  async function loadEvents() {
    ...
    if (err) {
      ...
      events = data || [];
      if (editingEventId && !events.find((event) => event.id === editingEventId)) {
        cancelEditEvent();
      }
    }
  }
  ...
  async function createRandomPickerForEvent(event: Event) {
    ...
    try {
      ...
      if (url) {
        ...
        flashSuccess("Random picker wheel created (check console for link)");
      }
    } catch (e: any) {
  ...
  function getCouncilName(councilId: string | null) {
    ...
    return councils.find((council) => council.id === councilId)?.name || "Unknown";
  }
  ...
  async function createCouncilAdmin() {
    if (!newAdminEmail.trim() || !newAdminPassword.trim() || !newAdminCouncilId) {
  ...
  {#if !user}
    <div class="max-w-md mx-auto pt-12">
      <div class="text-center mb-10 space-y-3">
        ...
        <div>
          <h1 class="font-['Nunito'] font-black text-[28px] text-[#020202]">
            Admin Access
          </h1>
      ...
      <form class="rounded-2xl border border-[#eaeaea] bg-white p-8 space-y-5" onsubmit={handleLogin}>
        {#if authError}
          <div class="rounded-xl border border-red-200 bg-red-50 p-3 text-center">
            <p class="font-['Inter'] text-[13px] text-red-700">{authError}</p>
          </div>
        {/if}

        <div class="space-y-2">
          <label for="email" class="font-['Inter'] text-[13px] font-medium text-[#555]"
            >Email</label
          >
          <input
            id="email"
        ...
        <div class="space-y-2">
          <label for="password" class="font-['Inter'] text-[13px] font-medium text-[#555]"
            >Password</label
          >
          <input
            id="password"
  ...
  {:else}
    <div class="space-y-6">
      <header class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="space-y-1">
          <p class="font-['Inter'] text-[13px] uppercase tracking-[0.12em] text-[#888]">
            {isSuperuser() ? "Superuser" : userCouncil ? userCouncil.name : "Council Admin"}
          </p>
          <h1 class="font-['Nunito'] font-black text-[32px] leading-none text-[#020202]">
            Dashboard
          </h1>
        ...
        {#if isSuperuser()}
          <div class="flex flex-wrap items-center gap-2">
            <label for="council-filter" class="font-['Inter'] text-[12px] text-[#666]">Filter</label>
            <select
              id="council-filter"
      ...
      <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-2xl border border-[#eaeaea] bg-white p-4">
          <p class="font-['Inter'] text-[12px] text-[#777]">Active events</p>
          <p class="font-['Nunito'] font-black text-[24px] text-[#222]">
            {events.filter((e) => e.is_active).length}
          </p>
        </div>
        <div class="rounded-2xl border border-[#eaeaea] bg-white p-4">
          <p class="font-['Inter'] text-[12px] text-[#777]">Total events</p>
          <p class="font-['Nunito'] font-black text-[24px] text-[#222]">
            {events.length}
          </p>
        </div>
        <div class="rounded-2xl border border-[#eaeaea] bg-white p-4">
          <p class="font-['Inter'] text-[12px] text-[#777]">Volunteers</p>
          <p class="font-['Nunito'] font-black text-[24px] text-[#222]">
            {volunteers.length}
          </p>
        </div>
        {#if isSuperuser()}
          <div class="rounded-2xl border border-[#eaeaea] bg-white p-4">
            <p class="font-['Inter'] text-[12px] text-[#777]">Councils</p>
            <p class="font-['Nunito'] font-black text-[24px] text-[#222]">
              {councils.length}
            </p>
          </div>
      ...
      {#if successMessage}
        <div class="rounded-2xl border border-[#4dfb59] bg-[#eafde3] p-4">
          <p class="font-['Inter'] text-[14px] text-[#20552a]">{successMessage}</p>
        </div>
      {/if}
      ...
      {#if loading}
        <div class="grid gap-6 xl:grid-cols-[2fr_1fr]">
          ...
          <div class="space-y-6">
            <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 animate-pulse">
              <div class="h-5 w-40 bg-neutral-200 rounded mb-2"></div>
              <div class="h-4 w-60 bg-neutral-200 rounded mb-6"></div>
              <div class="grid gap-4 sm:grid-cols-2">
                <div class="h-12 bg-neutral-100 rounded-[16px]"></div>
                <div class="h-12 bg-neutral-100 rounded-[16px]"></div>
                <div class="h-12 bg-neutral-100 rounded-[16px] sm:col-span-2"></div>
              </div>
              <div class="h-12 mt-6 bg-neutral-100 rounded-[16px]"></div>
            </div>
            <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 space-y-4 animate-pulse">
              <div class="flex items-baseline justify-between">
                <div class="h-5 w-28 bg-neutral-200 rounded"></div>
                <div class="h-3 w-16 bg-neutral-200 rounded"></div>
              </div>
              <div class="space-y-3">
                <div class="h-16 bg-neutral-100 rounded-xl"></div>
                <div class="h-16 bg-neutral-100 rounded-xl"></div>
                <div class="h-16 bg-neutral-100 rounded-xl"></div>
              </div>
            </div>
          ...
          <div class="space-y-6">
            <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 animate-pulse">
              <div class="h-5 w-28 bg-neutral-200 rounded mb-4"></div>
              <div class="space-y-2">
                <div class="h-11 bg-neutral-100 rounded-[16px]"></div>
                <div class="h-11 bg-neutral-100 rounded-[16px]"></div>
                <div class="h-11 bg-neutral-100 rounded-[16px]"></div>
              </div>
              <div class="h-10 mt-4 bg-neutral-100 rounded-[12px]"></div>
            </div>
            <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 animate-pulse">
              <div class="h-5 w-36 bg-neutral-200 rounded mb-4"></div>
              <div class="space-y-3">
                <div class="h-12 bg-neutral-100 rounded-[12px]"></div>
                <div class="h-12 bg-neutral-100 rounded-[12px]"></div>
                <div class="h-12 bg-neutral-100 rounded-[12px]"></div>
              </div>
            </div>
      ...
      {:else}
        <div class="grid gap-6 xl:grid-cols-[2fr_1fr]">
          <section class="space-y-6">
            <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 space-y-4">
              <div class="space-y-1">
                <h2 class="font-['Nunito'] font-black text-[20px] text-[#222]">Create an event</h2>
                <p class="font-['Inter'] text-[13px] text-[#777]">
                  Publish a new volunteer opportunity for your council
                </p>
              </div>
              ...
              {#if isSuperuser() && councils.length > 0}
                <div class="grid gap-3 sm:grid-cols-2">
                  <div class="space-y-2">
                    <label class="font-['Inter'] text-[13px] font-medium text-[#555]">Council</label>
                    <select
                  ...
                  <div class="space-y-2">
                    <label class="font-['Inter'] text-[13px] font-medium text-[#555]">Status</label>
                    <div class="h-12 flex items-center rounded-[16px] border border-[#f2f2f2] bg-[#fafafa] px-4 font-['Inter'] text-[13px] text-[#666]">
                      {events.filter((e) => e.council_id === newEventCouncilId).filter((e) => e.is_active).length} active in this council
                    </div>
                  </div>
                </div>
              {/if}
              ...
              {#if !isSuperuser() && userCouncil}
                <p class="font-['Inter'] text-[13px] text-[#777]">
                  Events will be created under <span class="font-semibold">{userCouncil.name}</span>.
                </p>
              {/if}
              ...
              <button
                ...
                onclick={createEvent}
                disabled={creatingEvent || !newEventName.trim() || (!isSuperuser() && !userCouncil) || (isSuperuser() && !newEventCouncilId)}
              >
            ...
            <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 space-y-4">
              <div class="flex items-baseline justify-between">
                <h2 class="font-['Nunito'] font-black text-[20px] text-[#222]">Events</h2>
                <span class="font-['Inter'] text-[13px] text-[#777]">
                  {(councilFilterId ? events.filter((e) => e.council_id === councilFilterId) : events).length} total
                </span>
              </div>

              {#if (councilFilterId ? events.filter((e) => e.council_id === councilFilterId) : events).length === 0}
                <div class="rounded-xl border border-[#f0f0f0] bg-[#fafafa] p-6 text-center">
                  <p class="font-['Nunito'] font-bold text-[16px] text-[#666]">
                    No events yet
                  </p>
                  <p class="font-['Inter'] text-[13px] text-[#9a9a9a]">
              ...
              {:else}
                <div class="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
                  {#each (councilFilterId ? events.filter((e) => e.council_id === councilFilterId) : events) as event (event.id)}
                    <div class="rounded-xl border border-[#f0f0f0] bg-white p-5 space-y-4">
                        ...
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                          <div class="space-y-2">
                            <div class="flex items-center gap-2">
                              ...
                              {/if}
                              <h3 class="font-['Nunito'] font-black text-[20px] text-[#222]">
                                {event.name}
                              </h3>
                            </div>
                            {#if isSuperuser()}
                              <p class="font-['Inter'] text-[13px] text-[#777]">
                                Council: <span class="font-semibold">{getCouncilName(event.council_id)}</span>
                              </p>
                            {/if}
                            {#if event.ends_at}
                              <p class="font-['Inter'] text-[13px] text-[#777]">
                          ...
                          <div class="flex flex-wrap gap-2">
                            ...
                            >
                              {event.is_active ? "Active" : "Set active"}
                            </button>
                            <button
                              type="button"
                      ...
                      <div class="flex flex-wrap gap-2">
                        ...
                        <button
                          ...
                          disabled={copySuccess === event.id || getVolunteersForEvent(event.id).length === 0}
                        >
                        ...
                        <button
                          ...
                          onclick={() => createRandomPickerForEvent(event)}
                          disabled={creatingWheelForEventId === event.id || getVolunteersForEvent(event.id).length === 0}
                          title="Create a random picker wheel using Wheel of Names"
                        >
                          {creatingWheelForEventId === event.id ? "Creating wheel..." : "Random Picker"}
                        </button>
                      </div>

                      {#if expandedEventId === event.id}
                        {#if getVolunteersForEvent(event.id).length === 0}
                          <div class="rounded-xl border border-[#f0f0f0] bg-[#fafafa] p-4 text-center">
                            <p class="font-['Inter'] text-[13px] text-[#777]">
                              No submissions yet
                            </p>
                          </div>
                        {:else}
                          <div class="space-y-2 rounded-xl border border-[#f0f0f0] bg-[#fafafa] p-4">
                            {#each getVolunteersForEvent(event.id) as volunteer (volunteer.id)}
                              <div class="flex flex-col gap-1 rounded-lg bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                  <p class="font-['Nunito'] font-bold text-[15px] text-[#222]">
                                    {volunteer.full_name}
                                  </p>
          ...
          <aside class="space-y-6 xl:sticky xl:top-6">
            {#if isSuperuser()}
              <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 space-y-4">
                <h2 class="font-['Nunito'] font-black text-[20px] text-[#222]">
                  Councils
                </h2>
                <div class="flex flex-col gap-3">
                  <div class="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={creatingCouncil || !newCouncilName.trim()}
                    >
                      {creatingCouncil ? "Adding..." : "Add"}
                    </button>
                  </div>
                  ...
                  {:else}
                    <ul class="space-y-2">
                      {#each councils as council (council.id)}
                        <li class="flex items-center justify-between rounded-lg border border-[#f2f2f2] bg-[#fafafa] px-4 py-3">
                          <span class="font-['Nunito'] text-[15px] text-[#333]">
                            {council.name}
                          </span>
                          <button
                            type="button"
                            ...
                            disabled={deletingCouncil === council.id}
                          >
                            {deletingCouncil === council.id ? "Deleting..." : "Delete"}
                          </button>
                        </li>
                      {/each}
                    </ul>
              ...
              <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 space-y-4">
                ...
                <div class="grid gap-3">
                  <div class="space-y-2">
                    <label class="font-['Inter'] text-[13px] text-[#555]" for="admin-email"
                      >Email</label
                    >
                    <input
                      id="admin-email"
                  ...
                  <div class="space-y-2">
                    <label class="font-['Inter'] text-[13px] text-[#555]" for="admin-password"
                      >Password</label
                    >
                    <input
                      id="admin-password"
                  ...
                  <div class="space-y-2">
                    <label class="font-['Inter'] text-[13px] text-[#555]" for="admin-council"
                      >Council</label
                    >
                    <select
                      id="admin-council"
                ...
                {:else}
                  <ul class="space-y-2">
                    {#each allAdmins as admin (admin.id)}
                      <li class="flex items-center justify-between rounded-lg border border-[#f2f2f2] bg-[#fafafa] px-4 py-3">
                        <div>
                          <p class="font-['Nunito'] text-[15px] text-[#333]">
                            {admin.email}
                          </p>
                          <p class="font-['Inter'] text-[12px] text-[#777]">
                            {admin.role === "superuser"
                              ? "Superuser"
                              : getCouncilName(admin.council_id)}
                          </p>
                        </div>
                        {#if admin.role !== "superuser"}
                          <button
                            type="button"
                            class="rounded-[10px] border border-[#ffd7d7] bg-[#fff0f0] px-3 py-1 font-['Nunito'] text-[12px] font-bold text-[#b63131] transition hover:bg-[#ffe0e0] disabled:opacity-50"
                            onclick={() => deleteCouncilAdmin(admin.id, admin.user_id)}
                            disabled={deletingAdmin === admin.id}
                          >
                            {deletingAdmin === admin.id ? "Removing..." : "Remove"}
                          </button>
                        {/if}
                      </li>
                    {/each}
            ...
            {:else}
              <div class="rounded-2xl border border-[#eaeaea] bg-white p-6 space-y-3">
                <h2 class="font-['Nunito'] font-black text-[20px] text-[#222]">
                  Your council
                </h2>
                {#if userCouncil}
                  <p class="font-['Inter'] text-[15px] text-[#333]">{userCouncil.name}</p>
                  <p class="font-['Inter'] text-[13px] text-[#777]">
                    {events.length} events · {volunteers.length} volunteer submissions
                  </p>
                {:else}
                  <p class="font-['Inter'] text-[14px] text-[#666]">
                    You aren't linked to a council yet. Ask a superuser to assign you.
                  </p>
                {/if}
              </div>
            {/if}
